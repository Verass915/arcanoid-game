...
let game = {
	...
	init() {  //код касающийся инициализации прил
		//init
		this.ctx = document.getElementById('mycanvas').getContext('2d');  //берем API для отрисовки, получ эл-т канвас
		this.setIvents();
	},

	setIvents() {
		window.addEventListener('keydown', e => { //проверяет событие и понимает какая клав зажата, нажатие кл
		...
		window.addEventListener('keyup', e => { //проверяет событие и понимает какая клав зажата, отпуск кл
			this.platform.stop();
		});
	},


	preload(callback) { //загружает все изображ в обхект спрайтс
		let loaded = 0;
		...
		for (let key in this.sprites) {
			...
			this.sprites[key].src = `img/${key}.png`; //загружаем картинку
	...
	render() { //отрисовка картинки. 1 парам-картинка, 2- координаты
